...
export default function QuickAddScreen() {
  ...
  const actions: Action[] = [
    ...
    { title: "Paper analysis", subtitle: "Record lesson-level performance and weakness signals.", icon: "analytics-outline", run: () => router.push("/paper-analysis") },
  ];

  return <View style={s.root}>
    <LinearGradient colors={["#1A1125", "#080D14"]} style={StyleSheet.absoluteFill} />
    <View style={s.header}><Pressable onPress={() => router.back()} style={s.back}><Ionicons name="arrow-back" size={21} color="#FFF" /></Pressable><View style={{ flex: 1 }}><Text style={s.title}>Quick add</Text><Text style={s.sub}>Capture something without hunting through the app.</Text></View></View>
    <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
      <View style={s.hero}><Ionicons name="flash-outline" size={25} color="#E6D1FF" /><View style={{ flex: 1 }}><Text style={s.heroTitle}>One place for common actions</Text><Text style={s.heroSub}>The fastest route should always be the default. Advanced controls remain available only when you need them.</Text></View></View>
