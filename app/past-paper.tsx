...
export default function PastPaperScreen() {
  ...
  const availableSubjects = useMemo(() => {
    ...
    return expanded.length ? expanded : ["Physics"];
  }, [profile.subjectChoices]);
  const subject = routeSubject && availableSubjects.includes(routeSubject) ? routeSubject : null;
  const configuredTopics = subject ? (((SUBJECTS as unknown as Record<string, any>)[subject]?.topics ?? []) as Array<{ id: string; title: string }>) : [];

  const openSubject = (subjectName: string) => router.push({ pathname: "/past-paper", params: { subjectName } });
  const openYears = (topicName: string) => {
    if (!subject) return;
    router.push({ pathname: "/past-paper-years", params: { subjectName: subject, topicName } });
  };

  useEffect(() => {
    if (!routeTopic || routeTopic === "Past Papers" || !routeSubject) return;
    router.replace({ pathname: "/past-paper-years", params: { subjectName: routeSubject, topicName: routeTopic } });
  }, [routeSubject, routeTopic, router]);
  ...
  return <View style={s.root}>
    ...
    <View style={s.header}>
      ...
      <View style={{ flex: 1 }}><Text style={s.kicker}>PAPER PRACTICE</Text><Text style={s.title}>{subject ? subject : "Past papers"}</Text><Text style={s.sub}>{subject ? "Choose full-paper practice or one lesson." : "Open a subject folder first. Full papers appear only inside that subject."}</Text></View>
    ...
    <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
      {!subject ? <>
        <View style={s.hero}>
          <View style={s.heroIcon}><Ionicons name="folder-open-outline" size={29} color="#E4D1F8" /></View>
          <View style={{ flex: 1 }}><Text style={s.heroTitle}>Choose a subject</Text><Text style={s.heroSub}>Past-paper years and the full-paper option stay inside the selected subject, so the first screen remains focused and uncluttered.</Text></View>
        </View>
        <Text style={s.label}>SUBJECT FOLDERS</Text>
        <View style={s.folderGrid}>{availableSubjects.map(item => <Pressable key={item} onPress={() => openSubject(item)} style={s.folder}>
          <View style={s.folderIcon}><Ionicons name="folder-outline" size={24} color="#D7BDF4" /></View>
          <Text style={s.folderTitle}>{item}</Text>
          <Text style={s.folderSub}>Full papers + lesson practice</Text>
          <Ionicons name="chevron-forward" size={18} color="#806D93" />
        </Pressable>)}</View>
      </> : <>
        <View style={s.hero}>
          <View style={s.heroIcon}><Ionicons name="documents" size={29} color="#E4D1F8" /></View>
          <View style={{ flex: 1 }}><Text style={s.heroTitle}>{subject} paper practice</Text><Text style={s.heroSub}>Choose the whole paper or a lesson. Each choice opens its own list of years.</Text></View>
        </View>

        <Text style={s.label}>FULL PAPER</Text>
        <Pressable onPress={() => openYears("General")} style={s.scopeFeatured}>
          <LinearGradient colors={["#382452", "#241833"]} style={s.scopeGradient}>
            ...
            <View style={{ flex: 1 }}><Text style={s.scopeKicker}>WHOLE SUBJECT</Text><Text style={s.scopeTitleFeatured}>Do a full paper</Text><Text style={s.scopeSubFeatured}>Choose a year, then the paper section used for this subject.</Text></View>
...
const s = StyleSheet.create({
  ...
  kicker: { color: "#A987D0", fontSize: 8, fontWeight: "900", letterSpacing: 1.4 }, title: { color: "#F7F5F9", fontSize: 23, fontWeight: "900", marginTop: 2 }, sub: { color: "#748194", fontSize: 9, marginTop: 2 },
  ...
  heroIcon: { width: 53, height: 53, borderRadius: 17, backgroundColor: "#B784FF18", alignItems: "center", justifyContent: "center" }, heroTitle: { color: "#F0EBF5", fontSize: 14, fontWeight: "900" }, heroSub: { color: "#8B8294", fontSize: 9.5, lineHeight: 15, marginTop: 4 },
  label: { color: "#7D899A", fontSize: 8.5, fontWeight: "900", letterSpacing: 1.3, marginTop: 22, marginBottom: 9 },
  folderGrid:{gap:9},folder:{minHeight:82,borderRadius:19,backgroundColor:"#101720",borderWidth:1,borderColor:"#293646",padding:12,flexDirection:"row",alignItems:"center",gap:11},folderIcon:{width:46,height:46,borderRadius:14,backgroundColor:"#B784FF14",alignItems:"center",justifyContent:"center"},folderTitle:{flex:1,color:"#EDF0F4",fontSize:13,fontWeight:"900"},folderSub:{color:"#6E7B8D",fontSize:8.5},
