  ...
  return <View style={s.root}>
    ...
    <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      <Animated.View style={{opacity:anim,transform:[{translateY:anim.interpolate({inputRange:[0,1],outputRange:[8,0]})}]}}>
        ...
        {step===8&&<Question eyebrow="DAILY STUDY TARGET" title="How many hours do you normally study in a full day?" subtitle="Choose from 1 to 12 hours. Class time counts toward this daily target by default, and Study Arc plans the remaining time as independent study."><View style={s.studyStepper}><Pressable onPress={()=>setSelfStudyHours(Math.max(1,selfStudyHours-.5))} style={s.roundStep}><Ionicons name="remove" size={27} color="#E3D3F8"/></Pressable><View style={{alignItems:"center"}}><Text style={s.studyHours}>{selfStudyHours.toFixed(1)}</Text><Text style={s.studyHoursLabel}>academic hours / day</Text></View><Pressable onPress={()=>setSelfStudyHours(Math.min(12,selfStudyHours+.5))} style={s.roundStep}><Ionicons name="add" size={27} color="#E3D3F8"/></Pressable></View><View style={s.tip}><Ionicons name="timer-outline" size={19} color="#B784FF"/><Text style={s.tipText}>When your day has enough uninterrupted space, Study Arc creates <Text style={s.tipStrong}>long self-directed study windows up to 3 hours</Text>. A 12-hour target can use large focus windows, while classes, meals, travel and protected time are placed around them. You can change these planning rules later.</Text></View></Question>}
