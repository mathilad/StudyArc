...
export default function ManualSessionScreen() {
  ...
  return <View style={s.root}>
    ...
    <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
      ...
      <Text style={s.label}>SESSION TYPE</Text><View style={s.wrap}>{TYPES.map(type => <Pressable key={type} onPress={() => setStudyType(type)} style={[s.chip, studyType === type && s.chipActive]}><Text style={[s.chipText, studyType === type && s.chipTextActive]}>{type === "Revision" ? "Revise" : type}</Text></Pressable>)}</View>
