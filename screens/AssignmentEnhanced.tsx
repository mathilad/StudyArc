    ...
    <View style={s.root}>
      ...
      <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <View style={s.priorityInfo}>
          <Ionicons name="flash-outline" size={20} color="#E7BC78" />
          <View style={{ flex: 1 }}>
            <Text style={s.priorityTitle}>Assignments have timetable priority</Text>
            <Text style={s.prioritySub}>Due work is divided across available study windows before normal lesson blocks.</Text>
          </View>
        </View>

        <Text style={s.section}>OPEN WORKLOAD</Text>
        {open.length ? open.map((assignment) => {
          const pct = progress[assignment.id] ?? 0;
