...
export default function AdminMonetizationScreen() {
  ...
  const { plans, updatePlanAdmin, setPaidModeAdmin, refreshMonetization } =
    useMonetization();
  ...
  const [bankName, setBankName] = useState(""),
    ...
    [instructions, setInstructions] = useState(
      "Include your Study Arc payment reference when making the transfer.",
    );
    ...
    try {
      const [
        ...
        supabase
          .from("payments")
          .select(
            "id,user_id,amount_lkr,payment_reference,status,receipt_path,submitted_at,subscription_plans(name)",
          )
          .eq("status", "PENDING")
          .order("submitted_at"),
  ...
  useEffect(() => {
    setPriceDrafts(
      Object.fromEntries(plans.map((p) => [p.id, String(p.priceLkr)])),
    );
  }, [plans]);
  if (!session) return <Redirect href="/login" />;
  if (!isAdmin) return <Redirect href="/(tabs)" />;

  const savePaidMode = async () => {
    if (!canConfigure) return;
    setBusy(true);
    try {
      await setPaidModeAdmin(paidMode, newOnly);
      setMessage("Paid Mode settings saved.");
    } catch (e) {
      setMessage(e instanceof Error ? e.message : "Could not save Paid Mode.");
    } finally {
      setBusy(false);
    }
  };
  const savePlan = async (
    ...
    if (!canConfigure) return;
    const price = Number(priceDrafts[planId]);
    if (!Number.isFinite(price) || price < 0) {
      setMessage("Enter a valid plan price.");
      return;
    }
    setBusy(true);
    try {
      await updatePlanAdmin(planId, price, enabled, featured);
      setMessage("Plan pricing updated.");
    } catch (e) {
      setMessage(e instanceof Error ? e.message : "Could not update plan.");
    } finally {
      setBusy(false);
    }
  };
  const addBank = async () => {
    if (!bankName.trim() || !holder.trim() || !account.trim()) {
      setMessage("Enter bank, account holder and account number.");
      return;
    }
    setBusy(true);
    try {
      const { error } = await supabase
        ...
        .insert({
          ...
          name: `${bankName.trim()} Bank Transfer`,
          ...
          enabled: true,
          display_order: methods.length * 10,
        });
      ...
      setAccount("");
      await load();
      setMessage("Bank payment method added.");
    } catch (e) {
      setMessage(
        e instanceof Error ? e.message : "Could not add bank account.",
      );
    } finally {
      setBusy(false);
    }
  ...
  const toggleMethod = async (m: PaymentMethod) => {
    ...
    try {
      const { error } = await supabase
        .from("payment_methods")
        .update({ enabled: !m.enabled, updated_at: new Date().toISOString() })
        .eq("id", m.id);
      if (error) throw error;
      await load();
    } catch (e) {
  ...
  const approve = async (id: string) => {
    ...
    try {
      ...
      if (error) throw error;
      await Promise.all([load(), refreshMonetization()]);
      setMessage("Payment approved and Premium activated.");
    } catch (e) {
      setMessage(e instanceof Error ? e.message : "Could not approve payment.");
    } finally {
      setBusy(false);
    }
  };
  const reject = (id: string) =>
    Alert.prompt
      ? Alert.prompt("Reject payment", "Optional reason", async (note) => {
          setBusy(true);
          try {
            ...
            setMessage(
              e instanceof Error ? e.message : "Could not reject payment.",
            );
          } finally {
            setBusy(false);
          }
        })
      : Alert.alert("Reject payment?", "Reject this pending payment?", [
  ...
  return (
    <View style={s.root}>
      ...
      <View style={s.head}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={s.title}>Monetization</Text>
          <Text style={s.sub}>
            Pricing, Paid Mode, bank details and payment approvals
          </Text>
        </View>
      </View>
      <ScrollView
      ...
      >
        ...
        <View style={s.grid}>
          <Metric
            ...
            value={money(Number(stats?.revenueThisMonth ?? 0))}
          />
          <Metric
            label="PENDING PAYMENTS"
            value={Number(stats?.pendingPayments ?? 0).toLocaleString()}
          />
          <Metric
            label="ACTIVE PREMIUM"
            value={Number(stats?.activePremium ?? 0).toLocaleString()}
          />
          <Metric
            label="JOURNEY USERS"
            value={Number(stats?.journeyUsers ?? 0).toLocaleString()}
          />
          <Metric
            label="EXPIRING IN 7 DAYS"
            value={Number(stats?.expiringIn7Days ?? 0).toLocaleString()}
          />
          <Metric
            label="BLOCKED USERS"
        ...
        <View style={s.card}>
          <Toggle
            title="Paid Mode"
            subtitle="When enabled, accounts that are not free/grandfathered/admin/premium are routed to plan activation."
            value={paidMode}
            onPress={() => canConfigure && setPaidMode((v) => !v)}
          />
          <Toggle
            title="New accounts only"
            subtitle="Keep users created before the Paid Mode cutoff grandfathered."
            value={newOnly}
            onPress={() => canConfigure && setNewOnly((v) => !v)}
          />
          <Pressable
            disabled={!canConfigure || busy}
            onPress={savePaidMode}
            style={[s.primary, (!canConfigure || busy) && { opacity: 0.5 }]}
          >
            <Text style={s.primaryText}>
              {canConfigure ? "Save Paid Mode" : "Super Admin only"}
            </Text>
          </Pressable>
        </View>

        <Text style={s.section}>PLAN PRICING</Text>
        <Text style={s.help}>
          These values are loaded from Supabase at runtime. Changing a price
          here changes the price students see without an app update.
        </Text>
        {plans.map((plan) => (
          <View key={plan.id} style={[s.planCard, plan.featured && s.featured]}>
            <View style={{ flex: 1 }}>
              <View style={s.row}>
            ...
            <View style={s.planControls}>
              <TextInput
                value={priceDrafts[plan.id] ?? String(plan.priceLkr)}
                onChangeText={(v) =>
                  setPriceDrafts((d) => ({ ...d, [plan.id]: v }))
                }
                keyboardType="numeric"
                style={s.priceInput}
              />
              <Text style={s.lkr}>LKR</Text>
              <Pressable
                disabled={!canConfigure || busy}
                onPress={() => savePlan(plan.id, !plan.enabled, plan.featured)}
                style={[s.smallButton, plan.enabled && s.smallButtonOn]}
              >
              ...
              <Pressable
                disabled={!canConfigure || busy}
                onPress={() => savePlan(plan.id, plan.enabled, !plan.featured)}
                style={[s.star, plan.featured && s.starOn]}
              >
                <Ionicons
        ...
        <Text style={s.section}>BANK / PAYMENT METHODS</Text>
        {methods.map((m) => (
          <View key={m.id} style={s.method}>
            <View style={{ flex: 1 }}>
              <Text style={s.methodTitle}>{m.name}</Text>
              <Text style={s.methodText}>
                {m.bankName}
                {m.branchName ? ` · ${m.branchName}` : ""}
              </Text>
              <Text style={s.methodText}>
                {m.accountHolder} · {m.accountNumber}
              </Text>
            </View>
            <Pressable
              onPress={() => toggleMethod(m)}
              style={[s.statusButton, m.enabled && s.statusButtonOn]}
            >
              <Text style={[s.statusText, m.enabled && s.statusTextOn]}>
        ...
        <View style={s.card}>
          <Text style={s.cardTitle}>Add bank account</Text>
          <Field label="BANK NAME" value={bankName} onChange={setBankName} />
          <Field label="BRANCH" value={branch} onChange={setBranch} />
          <Field label="ACCOUNT HOLDER" value={holder} onChange={setHolder} />
          <Field label="ACCOUNT NUMBER" value={account} onChange={setAccount} />
          <Field
        ...
        <Text style={s.section}>PENDING PAYMENTS</Text>
        {pending.length === 0 ? (
          <View style={s.empty}>
            <Ionicons
              ...
              color="#6FA78A"
            />
            <Text style={s.emptyText}>No payments waiting for review.</Text>
          </View>
        ) : (
          pending.map((p) => (
            <View key={p.id} style={s.payment}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={s.paymentMeta}>
                  {p.plan?.name ?? "Plan"} · {money(p.amount_lkr)}
                </Text>
                <Text style={s.paymentMeta}>
              ...
              <View style={s.paymentActions}>
                ...
                <Pressable
                  ...
                  style={s.approve}
                >
                  <Ionicons name="checkmark" size={17} color="#102016" />
                </Pressable>
                <Pressable
                  disabled={busy}
                  onPress={() => reject(p.id)}
                  style={s.reject}
                >
                  <Ionicons name="close" size={17} color="#FFDCE2" />
                </Pressable>
              </View>
            </View>
          ))
...
const s = StyleSheet.create({
  ...
  messageText: { color: "#8BD4A7", fontSize: 10.5, fontWeight: "700" },
  section: {
    color: "#8190A3",
    fontSize: 9,
    fontWeight: "900",
  ...
  cardTitle: { color: "#EBEDF2", fontSize: 13, fontWeight: "900" },
  toggleRow: {
    minHeight: 72,
    flexDirection: "row",
    alignItems: "center",
    gap: 12,
    borderBottomWidth: 1,
    borderBottomColor: "#25303D",
    paddingVertical: 10,
  },
  toggleTitle: { color: "#E9EDF2", fontSize: 12, fontWeight: "900" },
  toggleSub: { color: "#748194", fontSize: 9.5, lineHeight: 14, marginTop: 4 },
  switch: {
    width: 47,
    height: 27,
    borderRadius: 14,
  ...
  switchOn: { backgroundColor: "#704AA1" },
  knob: { width: 21, height: 21, borderRadius: 11, backgroundColor: "#9AA5B3" },
  knobOn: { marginLeft: 20, backgroundColor: "#FFF" },
  primary: {
    minHeight: 47,
    borderRadius: 14,
  ...
  featured: { borderColor: "#725099", backgroundColor: "#171321" },
  row: { flexDirection: "row", alignItems: "center", gap: 7 },
  planName: { color: "#EEF1F5", fontSize: 13, fontWeight: "900" },
  badge: {
    borderRadius: 7,
    backgroundColor: "#C193FF",
    paddingHorizontal: 6,
    paddingVertical: 3,
  },
  badgeText: { color: "#170C1F", fontSize: 6.5, fontWeight: "900" },
  planDesc: {
    color: "#718092",
    fontSize: 9.5,
    lineHeight: 14,
  ...
  lkr: { color: "#788596", fontSize: 8, fontWeight: "900" },
  smallButton: {
    height: 37,
    minWidth: 43,
    borderRadius: 10,
  ...
  smallButtonOn: { backgroundColor: "#153122" },
  smallButtonText: { color: "#9EA9B7", fontSize: 7.5, fontWeight: "900" },
  star: {
    width: 37,
    height: 37,
    borderRadius: 10,
  ...
  methodTitle: { color: "#E7EBEF", fontSize: 11, fontWeight: "900" },
  methodText: { color: "#768496", fontSize: 9.5, marginTop: 3 },
  statusButton: {
    minHeight: 34,
    borderRadius: 10,
    backgroundColor: "#29191E",
    paddingHorizontal: 8,
    alignItems: "center",
    justifyContent: "center",
  },
  statusButtonOn: { backgroundColor: "#153122" },
  statusText: { color: "#C18A95", fontSize: 7.5, fontWeight: "900" },
  statusTextOn: { color: "#83CBA0" },
  fieldLabel: {
    color: "#6E7C8F",
    fontSize: 7.5,
    fontWeight: "900",
  ...
  emptyText: { color: "#769083", fontSize: 10 },
  payment: {
    minHeight: 76,
    borderRadius: 17,
    backgroundColor: "#101720",
  ...
  paymentMeta: { color: "#748194", fontSize: 9, marginTop: 3 },
  paymentActions: { flexDirection: "row", gap: 5 },
  view: {
    width: 36,
    height: 36,
    borderRadius: 10,
