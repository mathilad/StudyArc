...
export default function WeeklyWorkloadBoard(){
 ...
 const repetitive=protectedTimes.filter(x=>(x.recurrence==="Daily"||x.recurrence==="Weekly")&&x.title.startsWith(TASK_PREFIX)).sort((a,b)=>a.recurrence.localeCompare(b.recurrence)||a.dayOfWeek-b.dayOfWeek||a.startTime.localeCompare(b.startTime));
 ...
 const repeatingRows=[...repetitive.map(x=>({id:x.id,title:x.title.slice(TASK_PREFIX.length),sub:`${x.recurrence==="Daily"?"Every day":DAYS[x.dayOfWeek]} · ${format12Hour(x.startTime)}–${format12Hour(x.endTime)}`,kind:"task" as const})),...classWork.map(x=>({id:x.id,title:x.title,sub:`${DAYS[x.day]} · ${x.sub}`,kind:x.kind})),{id:"day-review",title:"Review the day",sub:"Every day · 30 min before wind-down",kind:"review" as const}];
