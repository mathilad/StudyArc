...
import { queuePaperScan } from "../lib/paperScanQueue";
...
export default function AnswerSheetAnalysisScreen() {
  ...
  const queueOffline = async () => {
    if (!user) throw new Error("Sign in before saving a paper for offline scanning.");
    const id = await queuePaperScan({
      userId: user.id,
      paperDate,
    ...
    setOfflineMessage(`Saved ${pages.length} page${pages.length === 1 ? "" : "s"} on this device. StudyArc will scan this paper automatically when internet returns.`);
    setPages([]);
    setReference(null);
    setAnalysis(null);
    setScoreConfirmed(false);
  ...
  const analyse = async () => {
    ...
    try {
      if (!isOnline) {
        await queueOffline();
        Alert.alert("Paper saved offline", "The photos/files and test date are stored on this device. StudyArc will scan them automatically after internet returns.");
        return;
      }
    ...
    try {
      ...
      for (const row of scoredRows.slice(0, 200)) {
        ...
        if (user) {
          ...
          } catch { /* The normal intelligence result remains queued even if metadata enrichment fails. */ }
        }
      }
      ...
      if (adjustment != null) await setSubjectAdjustment(planningBucketFor(subject), adjustment);
      ...
      const change = adaptiveScore == null
        ? `Saved ${pages.length} ordered page${pages.length === 1 ? "" : "s"}.${classText} Reliable question marks and weak lessons were recorded.`
        : `Recorded ${num(obtainedText)}/${num(maximumText)} (${adaptiveScore}%, ${levelForScore(adaptiveScore)}).${classText} Study time changed by ${adj >= 0 ? "+" : ""}${Math.round(adj / 60 * 10) / 10}h/week and ${recalls.length} revision timer${recalls.length === 1 ? " was" : "s were"} updated.`;
  ...
  return <View style={s.root}>
    ...
    <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
      {!isOnline ? <View style={s.offline}><Ionicons name="cloud-offline-outline" size={21} color="#E4BE78"/><View style={{ flex: 1 }}><Text style={s.offlineTitle}>Offline paper upload is available</Text><Text style={s.offlineText}>Add photos or files and enter the test date now. Tap the scan button to queue the paper; StudyArc will process it after connectivity returns.</Text></View></View> : null}
      {offlineMessage ? <View style={s.queued}><Ionicons name="checkmark-circle-outline" size={20} color="#79D09C"/><Text style={s.queuedText}>{offlineMessage}</Text></View> : null}
      <View style={s.notice}><Ionicons name="layers-outline" size={21} color="#82CAA1"/><Text style={s.noticeText}>Page order matters. If a later page has no question number, StudyArc carries the question context from the previous page. Red teacher marks, written totals and question-level marks are cross-checked.</Text></View>
