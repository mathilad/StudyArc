...
export default function ExamsScreen() {
  ...
  const { exams, examComponents, addExam, deleteExam, addExamComponent, deleteExamComponent } = useAcademic();
  ...
  const removeComponent = async (id: string, key: string) => {
    setSaving(true); setMessage(null);
    try {
      await deleteExamComponent(id);
      setDrafts(current => { const next = { ...current }; delete next[key]; return next; });
      setMessage("Paper date deleted.");
    } catch (e) { setMessage(e instanceof Error ? e.message : "Could not delete paper date."); }
    finally { setSaving(false); }
  };

  const removeSelectedExam = async () => {
    if (!selectedExam) return;
    const nextExam = orderedExams.find(exam => exam.id !== selectedExam.id) ?? null;
    setSaving(true); setMessage(null);
    try {
      await deleteExam(selectedExam.id);
      setSelectedExamId(nextExam?.id ?? null);
      setMessage("Exam and its paper dates deleted.");
    } catch (e) { setMessage(e instanceof Error ? e.message : "Could not delete exam."); }
    finally { setSaving(false); }
  };
        ...
        {subjects.map(subject => <View key={subject} style={s.subjectCard}>
          ...
          {paperComponentsFor(subject).map(componentName => {
            ...
            return <View key={componentName} style={s.component}>
              ...
              {existing ? <Pressable disabled={saving} onPress={() => removeComponent(existing.id, key)} accessibilityLabel={`Delete ${subject} ${componentName} date`} style={s.deleteDate}><Ionicons name="trash-outline" size={17} color="#FF9AAB" /></Pressable> : null}
            </View>;
          })}
        </View>)}
