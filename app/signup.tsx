...
export default function SignupScreen() {
  ...
  const submit = async () => {
    setError(null);
    if (!email.trim() || !password || !confirmPassword) { setError("Enter your email and password to continue."); return; }
    ...
    if (result.needsEmailConfirmation) { setAccountCreated(true); return; }
  ...
  const resend = async () => {
    ...
    setResending(false);
    setError(result.error ?? "Verification email sent again. Check Inbox and Spam/Junk.");
  };
  ...
  if (accountCreated) {
    return <View style={s.page}><View style={s.card}>
      ...
      <View style={s.steps}>
        ...
        <Step n="2" text="Tap Confirm email address." />
  ...
  return <KeyboardAvoidingView style={s.page} behavior={Platform.OS === "ios" ? "padding" : undefined}><View style={s.card}>
    <View style={s.logo}><Text style={s.logoText}>Study<Text style={s.logoAccent}> Arc</Text></Text></View>
    <Text style={s.title}>Create account</Text>
    <Text style={s.subtitle}>Create your account first. Your academic setup comes after email verification.</Text>

    <Text style={s.label}>EMAIL</Text>
    <View style={s.inputWrap}><Ionicons name="mail-outline" size={18} color="#7D8796" /><TextInput value={email} onChangeText={setEmail} placeholder="you@example.com" placeholderTextColor="#555E6B" keyboardType="email-address" autoCapitalize="none" autoCorrect={false} style={s.input} /></View>

    <Text style={s.label}>PASSWORD</Text>
    <View style={s.inputWrap}><Ionicons name="lock-closed-outline" size={18} color="#7D8796" /><TextInput value={password} onChangeText={setPassword} placeholder="At least 8 characters" placeholderTextColor="#555E6B" secureTextEntry={!showPassword} autoCapitalize="none" style={s.input} /><Pressable onPress={() => setShowPassword(v => !v)} hitSlop={10}><Ionicons name={showPassword ? "eye-off-outline" : "eye-outline"} size={19} color="#7D8796" /></Pressable></View>

    <Text style={s.label}>CONFIRM PASSWORD</Text>
    <View style={s.inputWrap}><Ionicons name="shield-checkmark-outline" size={18} color="#7D8796" /><TextInput value={confirmPassword} onChangeText={setConfirmPassword} placeholder="Repeat password" placeholderTextColor="#555E6B" secureTextEntry={!showPassword} autoCapitalize="none" style={s.input} onSubmitEditing={submit} /></View>

    {error ? <Text style={s.error}>{error}</Text> : null}
    <Pressable style={[s.primary, submitting && s.disabled]} onPress={submit} disabled={submitting}>{submitting ? <ActivityIndicator color="#FFFFFF" /> : <Text style={s.primaryText}>CREATE ACCOUNT</Text>}</Pressable>
