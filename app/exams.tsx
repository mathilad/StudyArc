...
export default function ExamsScreen() {
  ...
  return <View style={s.root}>
    ...
    <View style={s.header}><Pressable onPress={() => router.back()} style={s.back}><Ionicons name="arrow-back" size={21} color="#FFF" /></Pressable><View style={{ flex: 1 }}><Text style={s.title}>Exams & paper dates</Text><Text style={s.sub}>Add school, mock and main exams with each subject paper separately.</Text></View></View>
    <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
      ...
      {selectedExam ? <>
        <View style={s.heading}><View style={{ flex: 1 }}><Text style={s.headingTitle}>{selectedExam.name}</Text><Text style={s.headingSub}>Enter each component independently. This lets Study Arc plan around MCQ, essay or Paper I / II dates instead of treating the subject as one date.</Text></View><Ionicons name="calendar-number-outline" size={26} color="#B784FF" /></View>
        {subjects.map(subject => <View key={subject} style={s.subjectCard}>
          <Text style={s.subject}>{subject}</Text>
          {paperComponentsForSubject(subject).map(componentName => {
            const key = `${selectedExam.id}::${subject}::${componentName}`;
            const existing = examComponents.find(row => row.examId === selectedExam.id && row.subjectName === subject && row.componentName === componentName);
