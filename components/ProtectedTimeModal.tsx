...
export default function ProtectedTimeModal({visible,onClose,onSave,mode="protected"}:Props){
  ...
  const [title,setTitle]=useState(repetitive?"Study routine":"Personal commitment");
  ...
  const save=async()=>{
    if(!title.trim()){setDialog({title:"Add a name",message:repetitive?"Give this task a short name, such as planning, cleaning, practice or flashcards.":"Give this protected time a short name, such as appointment, meal, club or family event."});return}
    if(parseTime(endTime)<=parseTime(startTime)){setDialog({title:"Check the time",message:"End time must be after start time."});return}
    setSaving(true);
    try{
      ...
      onClose();
    }catch(error){setDialog({title:"Could not save",message:error instanceof Error?error.message:"Please try again."})}
  ...
  return <>
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <View style={s.overlay}><View style={s.sheet}>
        <View style={s.header}><View><Text style={s.eyebrow}>{repetitive?"REPETITIVE TASK":"PROTECTED TIME"}</Text><Text style={s.title}>{repetitive?"Reserve repeating task time":"Block non-study time"}</Text></View><Pressable onPress={onClose} style={s.close}><Ionicons name="close" size={20} color="#FFF"/></Pressable></View>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{paddingBottom:24}}>
          <View style={s.info}><Ionicons name={repetitive?"repeat-outline":"shield-checkmark-outline"} size={19} color="#C8A5F5"/><Text style={s.infoText}>{repetitive?"Choose Daily for routines that need time every day, or Weekly for a task that repeats on one chosen weekday. StudyArc keeps that interval reserved before it builds study blocks.":"Use this for anything you must attend or cannot study during. StudyArc will leave this time unavailable instead of filling it with study."}</Text></View>
          ...
          <Pressable disabled={saving} onPress={save} style={[s.save,saving&&{opacity:.55}]}><Text style={s.saveText}>{saving?"Saving…":repetitive?`Add ${recurrence.toLowerCase()} task`:"Protect this time"}</Text></Pressable>
        </ScrollView>
      </View></View>
