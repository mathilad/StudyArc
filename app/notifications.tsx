...
import { markNotificationsRead } from "../lib/notificationReadState";
import { buildNotificationFeed } from "../lib/notifications";
import { generateDailyPlan } from "../lib/planner";

const iconFor = (kind: string): keyof typeof Ionicons.glyphMap => kind === "revision" || kind === "memory" ? "refresh-outline" : kind === "class_complete" ? "checkmark-done-outline" : kind === "class" ? "school-outline" : kind === "protected" ? "shield-checkmark-outline" : kind === "balance" ? "git-compare-outline" : kind === "exam" || kind === "phase" ? "flag-outline" : "timer-outline";
...
export default function NotificationsScreen() {
  ...
  return <View style={s.root}>
    ...
    <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
      <Text style={s.sub}>Study reminders, subject-balance warnings, protected commitments, phase suggestions, recall-due topics, classes, friend requests and exam alerts. Opening this page marks the current notifications as read.</Text>
