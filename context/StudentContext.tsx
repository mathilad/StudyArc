...
export function StudentProvider({ children }: { children: React.ReactNode }) {
  ...
  const addClass = useCallback(async (v: NewClass) => {
    if (!user) throw new Error("You must be signed in.");
    const local: ClassSchedule = { ...v, id: v.id ?? makeUuid(), travelMinutes: v.deliveryMode === "Physical" ? Math.max(0, v.travelMinutes) : 0 };
  ...
  const setSubtopicsCovered = useCallback(async (
    ...
    const changed: SubtopicCoverage[] = validNames.map(subtopicName => {
      const existing = subtopicCoverage.find(x => x.subjectName === subjectName && x.topicName === topicName && x.subtopicName === subtopicName && x.source === source);
      return {
    ...
    const nextCoverage = [
      ...subtopicCoverage.filter(x => !(x.subjectName === subjectName && x.topicName === topicName && names.has(x.subtopicName) && x.source === source)),
      ...changed,
    ];
    const total = Math.max(1, topic.subtopics.length);
    const coveredCount = topic.subtopics.filter(subtopic => nextCoverage.some(x => x.subjectName === subjectName && x.topicName === topicName && x.subtopicName === subtopic && x.source === "Manual" && x.covered)).length;
