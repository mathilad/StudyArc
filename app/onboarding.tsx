...
export default function OnboardingScreen(){
  ...
  if(generating)return <View style={s.buildRoot}>
    ...
    <View style={s.buildCard}>
      ...
      <Text style={s.buildText}>Study Arc is fitting your self-study target, subjects, classes and real daily routine together.</Text>
  ...
  return <View style={s.root}>
    ...
    <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      <Animated.View style={{opacity:anim,transform:[{translateY:anim.interpolate({inputRange:[0,1],outputRange:[8,0]})}]}}>
        ...
        {step===8&&<Question eyebrow="SELF-STUDY TARGET" title="How many hours do you want to self-study on a normal day?" subtitle="Choose up to 12 hours. Classes still appear in your timetable, but class time does not count toward this self-study target."><View style={s.studyStepper}><Pressable onPress={()=>setSelfStudyHours(Math.max(1,selfStudyHours-.5))} style={s.roundStep}><Ionicons name="remove" size={27} color="#E3D3F8"/></Pressable><View style={{alignItems:"center"}}><Text style={s.studyHours}>{selfStudyHours.toFixed(1)}</Text><Text style={s.studyHoursLabel}>self-study hours / day</Text></View><Pressable onPress={()=>setSelfStudyHours(Math.min(12,selfStudyHours+.5))} style={s.roundStep}><Ionicons name="add" size={27} color="#E3D3F8"/></Pressable></View><View style={s.tip}><Ionicons name="timer-outline" size={19} color="#B784FF"/><Text style={s.tipText}>When your day has enough uninterrupted space, Study Arc creates <Text style={s.tipStrong}>long self-directed study windows up to 3 hours</Text>. A 12-hour target can therefore become four large study windows, while classes, meals, travel and protected time stay separate.</Text></View></Question>}
