...
export default function ProtectedTimeModal({visible,onClose,onSave,mode="protected"}:Props){
  const repetitive=mode==="repetitive";
  const week=useMemo(()=>currentWeekDates(),[visible]);
  const [title,setTitle]=useState(repetitive?"Weekly task":"Personal commitment");
  const [recurrence,setRecurrence]=useState<ProtectedTimeInput["recurrence"]>(repetitive?"Weekly":"This Week");
  ...
  React.useEffect(()=>{if(!visible)return;setDayOfWeek(new Date().getDay());setRecurrence(repetitive?"Weekly":"This Week");setTitle(repetitive?"Weekly task":"Personal commitment")},[repetitive,visible]);

  const activeClockValue=clock==="end"?endTime:startTime;
  const activeClockTitle=clock==="end"?(repetitive?"Task ends":"Protected time ends"):(repetitive?"Task starts":"Protected time starts");
  const changeActiveClock=(value:string)=>{if(clock==="end")setEndTime(value);else if(clock==="start")setStartTime(value)};

  const save=async()=>{
    if(!title.trim()){Alert.alert("Add a name",repetitive?"Give this repetitive task a short name, such as weekly planning, cleaning or practice.":"Give this protected time a short name, such as appointment, meal, club or family event.");return}
    if(parseTime(endTime)<=parseTime(startTime)){Alert.alert("Check the time","End time must be after start time.");return}
    setSaving(true);
    try{
      const effectiveRecurrence=repetitive?"Weekly":recurrence;
      const date=effectiveRecurrence==="This Week"?localDateKey(week[dayOfWeek]):null;
      await onSave({title:title.trim(),recurrence:effectiveRecurrence,dayOfWeek,date,startTime,endTime});
      onClose();
  ...
  return <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
    <View style={s.overlay}><View style={s.sheet}>
      <View style={s.header}><View><Text style={s.eyebrow}>{repetitive?"REPETITIVE TASK":"PROTECTED TIME"}</Text><Text style={s.title}>{repetitive?"Reserve weekly task time":"Block non-study time"}</Text></View><Pressable onPress={onClose} style={s.close}><Ionicons name="close" size={20} color="#FFF"/></Pressable></View>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{paddingBottom:24}}>
        <View style={s.info}><Ionicons name={repetitive?"repeat-outline":"shield-checkmark-outline"} size={19} color="#C8A5F5"/><Text style={s.infoText}>{repetitive?"Choose a day and time that this task should receive every week. Study Arc keeps that interval separate from automatically planned study.":"Use this for anything you must attend or cannot study during. Study Arc will leave this time unavailable instead of filling it with study."}</Text></View>
        <Text style={s.label}>{repetitive?"TASK NAME":"NAME"}</Text>
        <TextInput value={title} onChangeText={setTitle} placeholder={repetitive?"Weekly planning, cleaning, practice…":"Appointment, club, family event…"} placeholderTextColor="#586577" style={s.input}/>
        {!repetitive&&<><Text style={s.label}>WHEN DOES IT APPLY?</Text><View style={s.modeRow}>{(["This Week","Weekly"] as const).map(x=><Pressable key={x} onPress={()=>setRecurrence(x)} style={[s.mode,recurrence===x&&s.modeActive]}><Ionicons name={x==="Weekly"?"repeat-outline":"calendar-outline"} size={19} color={recurrence===x?"#EEDFFF":"#7F8B9B"}/><Text style={[s.modeText,recurrence===x&&s.modeTextActive]}>{x}</Text></Pressable>)}</View></>}
        {repetitive&&<View style={s.weeklyBadge}><Ionicons name="repeat-outline" size={17} color="#DCC8F5"/><Text style={s.weeklyBadgeText}>Repeats every week</Text></View>}
        ...
        <Pressable disabled={saving} onPress={save} style={[s.save,saving&&{opacity:.55}]}><Text style={s.saveText}>{saving?"Saving…":repetitive?"Add weekly task":"Protect this time"}</Text></Pressable>
      </ScrollView>
    </View></View>
