...
const iconFor = (kind: string): keyof typeof Ionicons.glyphMap => kind === "revision" || kind === "memory" ? "refresh-outline" : kind === "class_complete" ? "checkmark-done-outline" : kind === "class" ? "school-outline" : kind === "exam" ? "flag-outline" : "timer-outline";

export default function NotificationsScreen() {
  const router = useRouter();
  const { profile, classes, topicProgress, testMarks, todayReview } = useStudent();
  const { pendingRequests } = useSocial();
  const today = useMemo(() => new Date(), []);
  const plan = useMemo(() => generateDailyPlan(today, profile, classes, topicProgress, testMarks), [classes, profile, testMarks, today, topicProgress]);
  ...
  return <View style={s.root}>
    ...
    <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
      <Text style={s.sub}>Today’s study reminders, recall-due topics, classes, friend requests and exam-phase alerts.</Text>
